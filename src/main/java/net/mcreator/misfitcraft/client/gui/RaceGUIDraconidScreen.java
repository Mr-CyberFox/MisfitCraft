...
public class RaceGUIDraconidScreen extends AbstractContainerScreen<RaceGUIDraconidMenu> implements MisfitcraftModScreens.ScreenAccessor {
	...
	protected void renderLabels(GuiGraphics guiGraphics, int mouseX, int mouseY) {
		guiGraphics.drawString(this.font, Component.translatable("gui.misfitcraft.race_gui_draconid.label_demon"), 24, 16, -12829636, false);
		guiGraphics.drawString(this.font, Component.translatable("gui.misfitcraft.race_gui_draconid.label_strength_high_stats"), 24, 97, -12829636, false);
	...
	public void init() {
		super.init();
		button_select = Button.builder(Component.translatable("gui.misfitcraft.race_gui_draconid.button_select"), e -> {
			int x = RaceGUIDraconidScreen.this.x;
			int y = RaceGUIDraconidScreen.this.y;
			if (true) {
				PacketDistributor.sendToServer(new RaceGUIDraconidButtonMessage(0, x, y, z));
				RaceGUIDraconidButtonMessage.handleButtonAction(entity, 0, x, y, z);
			}
		}).bounds(this.leftPos + 59, this.topPos + 172, 55, 20).build();
		this.addRenderableWidget(button_select);
		button_empty = Button.builder(Component.translatable("gui.misfitcraft.race_gui_draconid.button_empty"), e -> {
			int x = RaceGUIDraconidScreen.this.x;
			int y = RaceGUIDraconidScreen.this.y;
			if (true) {
				PacketDistributor.sendToServer(new RaceGUIDraconidButtonMessage(1, x, y, z));
				RaceGUIDraconidButtonMessage.handleButtonAction(entity, 1, x, y, z);
			}
		}).bounds(this.leftPos + -26, this.topPos + 70, 18, 20).build();
		this.addRenderableWidget(button_empty);
		button_empty1 = Button.builder(Component.translatable("gui.misfitcraft.race_gui_draconid.button_empty1"), e -> {
			int x = RaceGUIDraconidScreen.this.x;
			int y = RaceGUIDraconidScreen.this.y;
			if (true) {
				PacketDistributor.sendToServer(new RaceGUIDraconidButtonMessage(2, x, y, z));
				RaceGUIDraconidButtonMessage.handleButtonAction(entity, 2, x, y, z);
			}
